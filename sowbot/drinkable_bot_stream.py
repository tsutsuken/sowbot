...
class DrinkableBotStream(tweepy.StreamingClient):
    def __init__(
        self, _bearer_token, *, return_type=Response, wait_on_rate_limit=False, **kwargs
    ):
        print("init DrinkableBotStream")
        super().__init__(
            _bearer_token, return_type=Response, wait_on_rate_limit=False, **kwargs
        )
    ...
    def on_tweet(self, tweet):
        ...
        if tweet.author_id == verylong_common_bot_author_id:
            ...
            if tweet_text_token_transfer_complete in tweet.text:
                ...
                tweet_fetched = self.client.get_tweet(
                    tweet_id,
                ...
                if not command_info:
                    ...
                    return
                print(
                    f"command_info command: {command_info.command}, token_amount: {command_info.token_amount}"
                )

                # コマンドに応じる
                if command_info.command == "summon":
                    transferred_token_amount = command_info.token_amount
                    reply_target_tweet_id = (
                        tweet_fetched.data.id
                    )  # リプライ先は「ユーザが共通ボットに命令をしたツイート」
                    self.on_command_summon(
                        transferred_token_amount, reply_target_tweet_id
                    )
                else:
    ...
    def command_info(self, tweet_text):
        ...
        regex_summon = (
            re.escape(username_verylong_common_bot)
            + r" tip "
            + re.escape(username_drinkable_bot)
            + r" (\d+) summon"
        )
    ...
    def on_command_summon(self, transferred_token_amount, reply_target_tweet_id):
        ...
        # sow軍団を召喚する
        tweet_text = (
            f"command: summon, transferred_token_amount: {transferred_token_amount}"
        )
        self.client.create_tweet(
            text=tweet_text, in_reply_to_tweet_id=reply_target_tweet_id
        )
