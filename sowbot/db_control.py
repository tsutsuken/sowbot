...
def join_sow(arg_user_id, arg_screen_name, arg_tweet_date):
	"""
        ...
        Returns: 
            int: 成否等を返す。RESULT_SUCCESS 追加成功(0),RESULT_ERROR 追加失敗(1), 
			                 JOIN_SOW_RESULT_UPDATE 更新(2), JOIN_SOW_RESULT_ALREADY 追加済(3)
...
def promote_class(arg_screen_name):
	"""
        階級を昇格させる。
        ...
        Examples:
		    >>>promote_class(”@user01")
    """	
	print(f"call promote_class {arg_screen_name}")
	try:
		query = session.query(SowMembers).filter(SowMembers.screen_name==arg_screen_name).first()
		query.class_id += 1
		session.commit()
		return RESULT_SUCCESS
		
	except:
		print('[promote_class]失敗')
		return RESULT_ERROR

def demote_class(arg_screen_name):
	"""
        階級を降格させる。
        ...
        Returns: 
            int: 成否を返す。RESULT_SUCCESS 成功(0),RESULT_ERROR 失敗(1),
			               RESULT_LOWEST_CLASS 降格させようとしたらすでに最低クラスだった(2)

        Examples:
		    >>>demote_class(”@user01")
    """	
	print(f"call demote_class {arg_screen_name}")
	try:
		query = session.query(SowMembers).filter(SowMembers.screen_name==arg_screen_name).first()
		if query.class_id > 0:
			query.class_id -= 1
			...
			print('[demote_class]すでに最低ランク')
			return RESULT_LOWEST_CLASS
		
	except:
		print('[promote_class]失敗')
		return RESULT_ERROR

def get_status_id(arg_screen_name):
	"""
        入隊状態を取得する。
        Args: 
            arg_screen_name (str): ScreenName
        ...
        Returns: 
            int: status_idを返す。失敗時はRESULT_GET_ID_ERROR(-1)

        Examples:
		    >>>get_status_id(”@user01")
    """	
	print(f"call get_status_id {arg_screen_name}")
	try:
		status_id =session.query(SowMembers).filter(SowMembers.screen_name==arg_screen_name).first().status_id
		return status_id
		
	except:
		print('[get_status_id]失敗')
		return RESULT_GET_ID_ERROR

def get_class_id(arg_screen_name):
	"""
        階級を取得する。
        ...
        Returns: 
            int: class_idを返す。失敗時はRESULT_GET_ID_ERROR(-1)

        Examples:
		    >>>get_class_id(”@user01")
    """	
	print(f"call get_class_id {arg_screen_name}")
	try:
		class_id =session.query(SowMembers).filter(SowMembers.screen_name==arg_screen_name).first().class_id
		return class_id
		
	except:
		print('[get_class_id]失敗')
		return RESULT_GET_ID_ERROR
